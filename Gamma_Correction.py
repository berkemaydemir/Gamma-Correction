...
def main():
    parser = argparse.ArgumentParser(description='grup4')
    ...
    for path in img_paths:
        ...
        # We subtract the intensity component of the image
        YCrCb = cv2.cvtColor(img, cv2.COLOR_BGR2YCrCb)
        Y = YCrCb[:,:,0]
        # Determine the image is bright or faded
        threshold = 0.3
        exp_in = 112 # Expected average intensity
        M,N = img.shape[:2]
        mean_in = np.sum(Y/(M*N)) 
        t = (mean_in - exp_in)/ exp_in
        
        # Procedure for gamma correction of the image
        img_output = None
        if t < -threshold: # Operations for faded image
            print (name + ": Faded Image")
            result = process_dimmed(Y)
            YCrCb[:,:,0] = result
            img_output = cv2.cvtColor(YCrCb,cv2.COLOR_YCrCb2BGR)
        elif t > threshold:
            print (name + ": Bright Image") # Operations for bright image
